...
void orbital_stability() {
    ...
    scanf("%lf\n", &calc_speed);
    scanf("%lf", &fact_speed);
    if (fact_speed >= calc_speed){ // сложновато, можно взять разницу и сверить на отрицательность числа, и домножить на -1 (если < 0), и не придется столько ветвлений делать
...
void crew_division() {
    ...
    scanf("%d", &pers); // тут правильно тремя вложенными циклами для проверки всех-всех комбинаций, которые возможны
    if (pers % 2 == 0){
        f = 1;
        nums = pers / 2;
